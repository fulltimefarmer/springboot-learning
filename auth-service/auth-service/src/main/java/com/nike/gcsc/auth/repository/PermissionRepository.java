...
public interface PermissionRepository extends JpaRepository<Permission, Long> {
    ...
    @Query(value = "SELECT p.* FROM gcsc_permission p, middle_group_permission gp WHERE p.id = gp.permission_id AND gp.group_id = ?1 order by p.name", nativeQuery = true)
    List<Permission> findByGroupId(Long groupId);
    
    @Query(value = "SELECT p.* FROM gcsc_permission p, middle_group_permission gp WHERE p.id = gp.permission_id AND gp.group_id = ?1 and p.project_id=?2 order by p.name", nativeQuery = true)
    List<Permission> findByGroupIdAndProjectId(Long groupId, Long projectId);

    @Query(value = "SELECT DISTINCT u.*\n" +
            "FROM gcsc_permission u \n" +
            "LEFT JOIN middle_group_permission mgu ON u.id = mgu.permission_id\n" +
            "WHERE 1=1\n" +
            "AND mgu.group_id <> ?1 OR mgu.group_id IS NULL order by u.name", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT u.*\n" +
        ...
        "LEFT JOIN middle_group_permission mgu ON u.id = mgu.permission_id\n" +
        "WHERE 1=1\n" +
        "AND (mgu.group_id <> ?1 OR mgu.group_id IS NULL) and u.project_id=?2 order by u.name", nativeQuery = true)
    List<Permission> findExcludeByGroupIdAndProjectId(Long groupId, Long projectId);
